...
def train_one_fold(
    ...
    window_samples: int, stride_samples: int, num_T: int, num_S: int, hidden_dim: int
    ...
    # Init Model dynamically as Tsception
    model = Tsception(
        num_classes=2, 
        input_size=(1, config.CHANNELS, window_samples),
        sampling_rate=config.SFREQ,
        num_T=num_T, num_S=num_S, hidden=hidden_dim, dropout_rate=dropout
    ).to(DEVICE)
...
def objective(trial):
    # Parameter Search Space for Tsception
    window_sec = trial.suggest_categorical("window_sec", [2.0, 4.0, 5.0, 7.0])
    num_T = trial.suggest_categorical("num_T", [3, 6, 9])
    num_S = trial.suggest_categorical("num_S", [2, 4])
    hidden_dim = trial.suggest_categorical("hidden_dim", [16, 32, 64])
    dropout = trial.suggest_float("dropout", 0.3, 0.7)
    lr = trial.suggest_float("lr", 1e-4, 5e-3, log=True)

    window_samples = int(window_sec * config.SFREQ)
    stride_samples = int((window_sec / 2.0) * config.SFREQ) # 50% overlap
    
    results_dir = Path(config.BASE_DIR) / f"results_trial_{trial.number}"
    
    all_subjects = list(range(1, 33))
    seed = 42
    epochs = 40 # Moderate cap for Optuna search
    ...
    for subject_id in all_subjects:
        ...
        for outer_fold, (outer_train_ids, outer_test_ids) in enumerate(outer_splits):
            summary, _ = train_one_fold(
                ...
                epochs=epochs, results_dir=results_dir, lr=lr, dropout=dropout,
                window_samples=window_samples, stride_samples=stride_samples,
                num_T=num_T, num_S=num_S, hidden_dim=hidden_dim
            )
            all_f1_scores.append(summary["video_macro_f1"])

    trial_f1 = float(np.mean(all_f1_scores)) if all_f1_scores else 0.0
    return trial_f1

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--test-run", action="store_true", help="Run 1 trial on 1 subject for debugging.")
    args = parser.parse_args()

    study_name = "tsception_study"
    storage_uri = None if args.test_run else config.OPTUNA_DB_URI
    ...
    n_trials = 1 if args.test_run else config.N_OPTUNA_TRIALS
    
    # We redefine objective strictly for the test run hack, normally Optuna calls objective directly.
    if args.test_run:
